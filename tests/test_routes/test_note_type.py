...
def test_post_create_notification_type(client, dbsession, note_types, status_code, body):
    ...
    if status_code == status.HTTP_200_OK:
        response_model = NoteTypeGet.model_validate(response.json(), extra="forbid")
        exist_note_type = NoteType.query(session=dbsession).filter(NoteType.type_id == response_model.type_id).one_or_none()
